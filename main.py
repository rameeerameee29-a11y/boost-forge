...
def _csrf_protect():
    ...
    if not origin:
        # Some browsers strip Referer; require at least one
        abort(403)
    try:
        o = urlparse(origin)
        host_header = request.host
        if o.netloc and o.netloc != host_header:
            abort(403)
    except Exception:
        abort(403)
...
def save_order(user_email, order_type, title, details, amount=0, whatsapp=''):
    ...
    if not user_email:
        # Guest order: just save without balance interaction
        try:
            conn = sqlite3.connect('database.db')
            c = conn.cursor()
            c.execute("""INSERT INTO orders (user_email, order_type, title, details, amount, whatsapp, status, created_at, balance_applied)
                         VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, 0)""",
    ...
    try:
        ...
        if order_type != 'payment' and amount and amount > 0:
            c.execute("SELECT COALESCE(balance,0) FROM users WHERE email=?", (user_email,))
            row = c.fetchone()
            current_balance = float(row[0]) if row else 0.0
            if current_balance < amount:
                conn.close()
                return ('insufficient', current_balance)
            c.execute("UPDATE users SET balance=COALESCE(balance,0)-? WHERE email=?", (amount, user_email))
            applied = 1
        c.execute("""INSERT INTO orders (user_email, order_type, title, details, amount, whatsapp, status, created_at, balance_applied)
                     VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)""",
                  (user_email, order_type, title, details, amount, whatsapp,
...
def get_user_by_email(email):
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute("SELECT name, balance, user_id_tag, pin FROM users WHERE email=?", (email,))
    row = c.fetchone()
    conn.close()
    return row
...
def login():
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        c.execute("SELECT email, pin, password FROM users WHERE email=?", (email,))
        row = c.fetchone()
        ok = False
        if row:
            ...
            if _is_hashed(stored_pw):
                ...
                if stored_pw == password:
                    ...
                    c.execute("UPDATE users SET password=? WHERE email=?", (new_hash, user_email))
                    conn.commit()
        conn.close()
        if ok:
...
def verify_pin():
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        c.execute("SELECT pin FROM users WHERE email=?", (pending,))
        row = c.fetchone()
        conn.close()
        ...
        if not ok:
            ...
            if action == 'enable':
                ...
                if not pin.isdigit() or len(pin) < 4 or len(pin) > 8:
                    ...
                    c.execute("UPDATE users SET pin=? WHERE email=?", (pin, email))
                    conn.commit()
                    current_pin = pin
                    msg = "تم تفعيل التحقق بخطوتين بنجاح"
            elif action == 'disable':
                c.execute("UPDATE users SET pin=NULL WHERE email=?", (email,))
                conn.commit()
                current_pin = None
...
def register():
    if request.method == 'POST':
        ...
        try:
            conn = sqlite3.connect('database.db')
            c = conn.cursor()
            c.execute("INSERT INTO users (user_id_tag, name, email, password) VALUES (?, ?, ?, ?)",
...
def my_orders():
    ...
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute("""SELECT id, order_type, title, details, amount, status, created_at, admin_note
                 FROM orders WHERE user_email=? ORDER BY id DESC""", (email,))
    rows = c.fetchall()
    conn.close()
...
def admin_users():
    ...
    if request.method == 'POST':
        ...
        if action == 'set' and uid:
            ...
            try:
                ...
                c.execute("UPDATE users SET balance=COALESCE(balance,0)+? WHERE id=?", (delta, uid))
                conn.commit()
    ...
    users = c.fetchall()
    conn.close()
    return render_template('admin_users.html', users=users, q=q, msg=msg, err=err)
...
def admin_orders():
    ...
    if request.method == 'POST':
        ...
        if action == 'note' and oid:
            ...
            c.execute("UPDATE orders SET admin_note=? WHERE id=?", (note, oid))
            conn.commit()
        else:
            new_status = request.form.get('status')
            if oid and new_status in ('pending', 'success', 'cancelled'):
                c.execute("SELECT user_email, amount, status, order_type, COALESCE(balance_applied,0) FROM orders WHERE id=?", (oid,))
                row = c.fetchone()
                if row:
                    ...
                    if o_email and o_amount > 0:
                        if o_type == 'payment':
                            # Top-up: credit on success, reverse if leaving success
                            if new_status == 'success' and not o_applied:
                                c.execute("UPDATE users SET balance=COALESCE(balance,0)+? WHERE email=?", (o_amount, o_email))
                                c.execute("UPDATE orders SET balance_applied=1 WHERE id=?", (oid,))
                            elif new_status != 'success' and o_applied:
                                c.execute("UPDATE users SET balance=COALESCE(balance,0)-? WHERE email=?", (o_amount, o_email))
                                c.execute("UPDATE orders SET balance_applied=0 WHERE id=?", (oid,))
                        else:
                            # Service order: deducted at order time. Refund on cancel; re-deduct if uncancelled.
